...
impl Messages {
    ...
    pub fn register<T: Message>(&mut self) {
        if let Err(i) = self.channels.binary_search_by_key(&TypeId::of::<T>(), |x| x.0) {
            self.channels.insert(i, (TypeId::of::<T>(), Box::new(Channel::<T>::new())));
        }
    }

    pub fn get_or_register<T: Message>(&mut self) -> &Channel<T> {
        let search = self.channels.binary_search_by_key(&TypeId::of::<T>(), |x| x.0);
        match search {
            Ok(i) => unsafe {
                &*(self.channels.get_unchecked_mut(i).1.as_mut() as *const dyn Any as *const Channel<T>)
            }
            Err(i) => {
                self.channels.insert(i, (TypeId::of::<T>(), Box::new(Channel::<T>::new())));
                unsafe {
    ...
    pub fn get<T: Message>(&self) -> &Channel<T> {
        let channel_idx = self.channels.binary_search_by_key(&TypeId::of::<T>(), |x| x.0).unwrap();
        unsafe {
            &*(self.channels.get_unchecked(channel_idx).1.as_ref() as *const dyn Any as *const Channel<T>)
        }
    }
    ...
    pub fn messages<T: Message>(&self) -> &[T] {
       ...
       if let Ok(channel_idx) = search {
           unsafe {
               (&*(self.channels.get_unchecked(channel_idx).1.as_ref() as *const dyn Any as *const Channel<T>)).messages()
           }
       } else {
           &[]
       }
    ...
    pub fn flush<T: Message>(&mut self) {
        ...
        if let Ok(channel_idx) = search {
            unsafe {
                (&*(self.channels.get_unchecked(channel_idx).1.as_ref() as *const dyn Any as *const Channel<T>)).flush()
            }
        }
    }
    ...
    pub fn sync<T: Message>(&mut self) {
        ...
        if let Ok(channel_idx) = search {
            unsafe {
                (&*(self.channels.get_unchecked(channel_idx).1.as_ref() as *const dyn Any as *const Channel<T>)).sync()
            }
        }
    }
...
impl<'a> UnsafeMessagesCell<'a> {
    ...
    pub fn get<T: Message>(&self) -> &'a Channel<T> {
        self.0.get()
    }
    ...
    pub unsafe fn messages<T: Message>(&self) -> &'a [T] {
        self.0.messages()
    }

    pub unsafe fn flush<T: Message>(&self) {
        let search = self.0.channels.binary_search_by_key(&TypeId::of::<T>(), |x| x.0);
        if let Ok(channel_idx) = search {
            unsafe {
                (&*(self.0.channels.get_unchecked(channel_idx).1.as_ref() as *const dyn Any as *const Channel<T>)).flush()
            }
        }
    }

    pub unsafe fn sync<T: Message>(&self) {
        let search = self.0.channels.binary_search_by_key(&TypeId::of::<T>(), |x| x.0);
        if let Ok(channel_idx) = search {
            unsafe {
                (&*(self.0.channels.get_unchecked(channel_idx).1.as_ref() as *const dyn Any as *const Channel<T>)).sync()
            }
        }
    }
}
