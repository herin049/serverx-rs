...
impl From<u8> for LightLevel {
    fn from(value: u8) -> Self {
        LightLevel(cmp::min(value, 15))
    }
}
...
impl LightSection {
    pub fn new(light_level: LightLevel) -> Self {
        Self {
            occupied: 0,
            lighting: vec![(light_level.0 | (light_level.0 << 4)); Chunk::WIDTH / 2],
        }
    }
    ...
    pub fn from_vec(lighting: Vec<u8>) -> Self {
        ...
        Self { occupied, lighting }
    }

    pub fn fill(&mut self, level: LightLevel) {
        self.occupied = if level.0 == 0 { 0 } else { Chunk::WIDTH as u16 };
    ...
    pub fn set(&mut self, pos: (usize, usize, usize), level: LightLevel) -> LightLevel {
        ...
        self.lighting[index] =
            self.lighting[index] & (0xf >> ((index % 2) * 4)) | (level.0 << ((index % 2) * 4));
        prev
    }
...
impl ChunkLighting {
    pub fn new(light_level: LightLevel, sections: usize) -> Self {
        let mut block_light = Vec::with_capacity(sections);
        let mut sky_light = Vec::with_capacity(sections);
        for _ in 0..sections {
            block_light.push(LightSection::new(light_level));
            sky_light.push(LightSection::new(light_level));
        }
        Self {
            block_light,
            sky_light,
    ...
    pub fn height(&self) -> usize {
        self.block_light.len() * Chunk::WIDTH
    }
    ...
    pub fn get_block_light(&self, pos: (usize, usize, usize)) -> LightLevel {
        self.block_light[pos.1 / Chunk::WIDTH].get((pos.0, pos.1 & 0xf, pos.2))
    }

    pub fn set_block_light(&mut self, pos: (usize, usize, usize), level: LightLevel) -> LightLevel {
        self.block_light[pos.1 / Chunk::WIDTH].set((pos.0, pos.1 & 0xf, pos.2), level)
    }

    pub fn get_sky_light(&self, pos: (usize, usize, usize)) -> LightLevel {
        self.sky_light[pos.1 / Chunk::WIDTH].get((pos.0, pos.1 & 0xf, pos.2))
    }

    pub fn set_sky_light(&mut self, pos: (usize, usize, usize), level: LightLevel) -> LightLevel {
        self.sky_light[pos.1 / Chunk::WIDTH].set((pos.0, pos.1 & 0xf, pos.2), level)
    }
}
